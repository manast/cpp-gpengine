...
ASTNode *LogicASTCreator::getASTNode (const Symbol *reduction, ASTNode *parent) {
    ...
    if (reduction->type == NON_TERMINAL) {
        ...
        */
        ...
        if (ind == RULE_AXIOMLIST) {
            ...
            getASTNodeList (children, (NonTerminal*) rdcChildren[1], 1, 2, axiomList); 
            return axiomList;
        }
        ...
        if (ind == RULE_DOUBLEIMPLICATION_LTMINUSGT) {
            ...
            doubleImplication->addChild (getASTNode (rdcChildren[0], doubleImplication));
            doubleImplication->addChild (getASTNode (rdcChildren[2], doubleImplication));
